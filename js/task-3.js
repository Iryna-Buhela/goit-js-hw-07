// Напиши скрипт, який під час набору тексту в інпуті input#name -
// input(подія input) підставляє його поточне значення в span#name -
// output як ім’я для привітання.Обов’язково очищай значення в інпуті
// по краях від пробілів.Якщо інпут порожній або містить лише пробіли,
// то замість імені у спан має підставлятися рядок "Anonymous".

const inputElem = document.querySelector("#name-input");
const outputElem = document.querySelector("#name-output");

function handleUserNameOutput(e) {
  const userValue = e.target.value.trim();

  if (userValue === "") {
    outputElem.textContent = "Anonymous";
  } else {
    outputElem.textContent = userValue;
  }
}

inputElem.addEventListener("input", handleUserNameOutput);

